(function() {
  ...
  function step(delta) {
    ...
    previous.transition().delay(750).duration(0).attr("src", slides[previousIndex] + thequery);

    thequery = query;
    if(currentIndex == 1) {
      thequery = "";
    }
    current.transition().delay(250).duration(0).attr("src", slides[currentIndex] + thequery);

    thequery = query;
    if(nextIndex == 1) {
      thequery = "";
    }
    next.transition().delay(750).duration(0).attr("src", slides[nextIndex] + thequery);
  }
})();
